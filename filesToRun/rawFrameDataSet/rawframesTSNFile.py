...
model = dict(  # Config of the model
    ...
    backbone=dict(  # Dict for backbone
        ...
        norm_eval=False),  # Whether to set BN layers to eval mode when training MAYBE SHOULD CHANGE TO TRUE!!!
...
# dataset settings NOTICE: this works only when running train from Operation-Detection folder (which one was manualOperationProject)
...
data_set_dir = os.path.join(current_path, "mmaction2/DataSet/")
...
train_dataloader = dict(  # Config of train dataloader
    ...
    num_workers=4,  # Workers to pre-fetch data for each single GPU during training
    ...
    dataset=dict(  # Config of train dataset
        ...
        data_prefix=dict(img=train_dir),  # Prefix of frame path
        pipeline=train_pipeline))
val_dataloader = dict(  # Config of validation dataloader
    batch_size=1,  # Batch size of each single GPU during validation
    num_workers=4,  # Workers to pre-fetch data for each single GPU during validation
    ...
    dataset=dict(  # Config of validation dataset
        ...
        data_prefix=dict(img=val_dir),  # Prefix of frame path
        pipeline=val_pipeline,
        test_mode=True))
test_dataloader = dict(  # Config of test dataloader
    batch_size=16,  # Batch size of each single GPU during testing
    num_workers=4,  # Workers to pre-fetch data for each single GPU during testing
    persistent_workers=True,  # If `True`, the dataloader will not shut down the worker processes after an epoch end
    sampler=dict(
        type='DefaultSampler',
        shuffle=False),  # Not shuffle during validation and testing
    dataset=dict(  # Config of test dataset
        type=dataset_type,
        ann_file=ann_file_val,  # Path of annotation file
        data_prefix=dict(img=test_dir),  # Prefix of frame path
